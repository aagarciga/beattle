export enum HttpStatusCodes {
   ...
   * before sending the body. If the client receives an error code such as 403 (Forbidden) or
   * 405 (Method Not Allowed) then it shouldn't send the request's body. The response 417 Expectation Failed
   * indicates that the request should be repeated without the Expect header as it indicates that the server
  ...
  MovedPermanently,
  /** Tells the client to look at (browse to) another url. 302 has been superseded by 303 and 307. This is an
   * example of industry practice contradicting the standard. The HTTP/1.0 specification (RFC 1945) required
  ...
  Found,
  /** The response to the request can be found under another URI using the GET method. When received in response
   * to a POST (or PUT/DELETE), the client should presume that the server has received the data and should issue
   * a new GET request to the given URI. */
  SeeOther,
   ...
   * Many HTTP clients (such as Mozilla Firefox and Internet Explorer) do not correctly handle responses with
   ...
   * the original URI. In contrast to how 302 was historically implemented, the request method is not allowed to
   * be changed when reissuing the original request. For example, a POST request should be repeated using
   * another POST request. */
  TemporaryRedirect,
  /** The request and all future requests should be repeated using another URI. 307 and 308 parallel the
   * behaviors of 302 and 301, but do not allow the HTTP method to change. So, for example, submitting a form
   ...
   * authentication. 401 semantically means "unauthenticated", i.e. the user does not have the
   * necessary credentials. */
  Unauthorized,
  /** Reserved for future use. The original intention was that this code might be used as part of some
   * form of digital cash or micropayment scheme, as proposed for example by GNU Taler, but that has not
   * yet happened, and this code is not usually used. Google Developers API uses this status if a
   * particular developer has exceeded the daily limit on requests. Sipgate uses this code if an
   * account does not have sufficient funds to start a call. Shopify uses this code when the store
   * has not paid their fees and is temporarily disabled.  */
  PaymentRequired,
  ...
  NotExtended = 510,
  /** The client needs to authenticate to gain network access. Intended for use by intercepting proxies used
